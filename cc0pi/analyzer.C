  ...
  bool create = false)
{
  ...
  set_output_branch_address( out_tree, "mc_is_cc0pi_wc_signal",
    &ev.mc_is_cc0pi_signal_, create, "mc_is_cc0pi_wc_signal/O" );
...
// Fill corrected muon reco from a muon candidate at idx p
void AnalysisEvent::apply_corrections(int p) {
  // Apply momentum corrections
  float muon_mom = LOW_FLOAT;
  bool mu_quality = true;
  ...
  if (sel_muon_contained_) {
    ...
    if (range_muon_mom > 0) {
      ...
      mu_quality = frac_diff_range_mcs < MUON_MOM_QUALITY_CUT;
    }
    muon_mom = range_muon_mom;
  }
  else {
    muon_mom = track_mcs_mom_mu_->at(p);
  }

  p3_mag_corr_mu_ = \
    PMuCorrection::correct(muon_mom, sel_muon_contained_, mu_quality);

  // TODO Apply costheta corrections
  float px = track_dirx_->at(p);
  float py = track_diry_->at(p);
  float pz = track_dirz_->at(p);
  p3_costheta_corr_mu_ = TVector3(px, py, pz).CosTheta();
}
...
void AnalysisEvent::apply_selection() {
  ...
  for ( int p = 0; p < num_pf_particles_; ++p ) {
    ...
    if ( p == muon_candidate_idx_ ) {
      ...
      // Fill corrected muon reco variables
      apply_corrections(p);

      // Check that the muon candidate is above threshold.
      float muon_mom = p3_mag_corr_mu_;
