...
constexpr float MUON_P_MIN_WC_MOM_CUT = 0.121; // GeV/c
constexpr float MUON_P_MAX_MOM_CUT = 2.000; // GeV/c
constexpr float CHARGED_PI_MOM_CUT = 0.07; // GeV/c
constexpr float CHARGED_PI_WC_MOM_CUT = 0.161; // GeV/c
...
class AnalysisEvent {
  ...
  public:
    ...
    int mc_num_protons_ = 0;
    int mc_num_neutrons_ = 0;
    int mc_num_charged_pions_ = 0;
    int mc_num_wc_charged_pions_ = 0;

    // Water Cherenkov cuts
    bool mc_muon_in_wc_mom_range_ = false;
    bool mc_no_charged_pi_above_wc_threshold_ = false;
    bool mc_is_cc0pi_wc_signal_ = false;
    ...
    bool sel_muon_passed_wc_mom_cuts_ = false;
    ...
    bool sel_presel_ = false;

    int sel_n_bdt_other_ = 0;
    int sel_n_bdt_muon_ = 0;
    int sel_n_bdt_pion_ = 0;
    int sel_n_bdt_proton_ = 0;
    int sel_n_bdt_invalid_ = 0;

    int sel_num_proton_candidates_ = 0;
    int sel_num_pion_candidates_ = 0;
    int sel_num_pion_wc_candidates_ = 0;
    bool sel_has_pion_candidate_ = false;
    bool sel_has_pion_wc_candidate_ = false;
    bool sel_CC0pi_ = false;
    bool sel_CC0pi_wc_ = false;
    ...
    float p3_mag_corr_mu_ = BOGUS;
    float p3_costheta_corr_mu_ = BOGUS;
...
void set_event_branch_addresses(TTree& etree, AnalysisEvent& ev)
{
 ...
 set_object_input_branch_address( etree, "trk_bragg_mu_fwd_preferred_v",
  ...
  set_object_input_branch_address( etree, "trk_bragg_mu_alt_dir_v",
      ev.trk_bragg_mu_alt_dir_v_ );

  set_object_input_branch_address( etree, "trk_bragg_p_fwd_preferred_v",
      ev.trk_bragg_p_fwd_preferred_v_ );

  set_object_input_branch_address( etree, "trk_bragg_p_alt_dir_v",
      ev.trk_bragg_p_alt_dir_v_ );

  set_object_input_branch_address( etree, "trk_bragg_mu_v",
      ev.trk_bragg_mu_v_ );

   set_object_input_branch_address( etree, "trk_bragg_p_v",
      ev.trk_bragg_p_v_ );

   set_object_input_branch_address( etree, "trk_bragg_pion_v",
      ev.trk_bragg_pion_v_ );

   set_object_input_branch_address( etree, "trk_bragg_mip_v",
      ev.trk_bragg_mip_v_ );

  set_object_input_branch_address( etree, "trk_pid_chimu_v",
      ev.track_chi2_muon_ );

  set_object_input_branch_address( etree, "trk_pid_chika_v",
      ev.track_chi2_kaon_ );
 
  set_object_input_branch_address( etree, "trk_pid_chipi_v",
      ev.track_chi2_pion_ );
  ...
  bool create = false)
{
  ...
  set_output_branch_address( out_tree, "mc_muon_in_wc_mom_range",
    &ev.mc_muon_in_mom_range_, create, "mc_muon_in_wc_mom_range/O" );
  ...
  set_output_branch_address( out_tree, "mc_no_charged_pi_above_wc_threshold",
    &ev.mc_no_charged_pi_above_wc_threshold_, create,
    "mc_no_charged_pi_above_wc_threshold/O" );

  set_output_branch_address( out_tree, "mc_num_protons",
    &ev.mc_num_protons_, create, "mc_num_protons/I" );

  set_output_branch_address( out_tree, "mc_num_neutrons",
    &ev.mc_num_neutrons_, create, "mc_num_neutrons/I" );

  set_output_branch_address( out_tree, "mc_num_charged_pions",
    &ev.mc_num_charged_pions_, create, "mc_num_charged_pions/I" );

  set_output_branch_address( out_tree, "mc_num_wc_charged_pions",
    &ev.mc_num_wc_charged_pions_, create, "mc_num_wc_charged_pions/I" );
  ...
  set_output_branch_address( out_tree, "mc_is_cc0pi_signal",
    &ev.mc_is_cc0pi_signal_, create, "mc_is_cc0pi_signal/O" );

  set_output_branch_address( out_tree, "mc_is_cc0pi_wc_signal",
    &ev.mc_is_cc0pi_wc_signal_, create, "mc_is_cc0pi_wc_signal/O" );
  ...
  set_output_branch_address( out_tree, "sel_muon_passed_wc_mom_cuts",
    &ev.sel_muon_passed_wc_mom_cuts_, create, "sel_muon_passed_wc_mom_cuts/O" );
  ...
  // XGBoost outputs
  set_output_branch_address( out_tree, "sel_n_bdt_other",
    &ev.sel_n_bdt_other_, create, "sel_n_bdt_other/I" );

  set_output_branch_address( out_tree, "sel_n_bdt_muon",
    &ev.sel_n_bdt_muon_, create, "sel_n_bdt_muon/I" );

  set_output_branch_address( out_tree, "sel_n_bdt_pion",
    &ev.sel_n_bdt_pion_, create, "sel_n_bdt_pion/I" );

  set_output_branch_address( out_tree, "sel_n_bdt_proton",
    &ev.sel_n_bdt_proton_, create, "sel_n_bdt_proton/I" );

  set_output_branch_address( out_tree, "sel_n_bdt_invalid",
    &ev.sel_n_bdt_invalid_, create, "sel_n_bdt_invalid/I" );

  // CC0pi
  set_output_branch_address( out_tree, "sel_presel",
    &ev.sel_presel_, create, "sel_presel/O" );

  set_output_branch_address( out_tree, "sel_num_proton_candidates",
    &ev.sel_num_proton_candidates_, create, "sel_num_proton_candidates/I" );

  set_output_branch_address( out_tree, "sel_num_pion_candidates",
    &ev.sel_num_pion_candidates_, create, "sel_num_pion_candidates/I" );

  set_output_branch_address( out_tree, "sel_num_pion_wc_candidates",
    &ev.sel_num_pion_wc_candidates_, create, "sel_num_pion_wc_candidates/I" );

  set_output_branch_address( out_tree, "sel_has_pion_candidate",
    &ev.sel_has_pion_candidate_, create, "sel_has_pion_candidate/O" );

  set_output_branch_address( out_tree, "sel_has_pion_wc_candidate",
    &ev.sel_has_pion_wc_candidate_, create, "sel_has_pion_wc_candidate/O" );

  set_output_branch_address( out_tree, "sel_CC0pi",
    &ev.sel_CC0pi_, create, "sel_CC0pi/O" );

  set_output_branch_address( out_tree, "sel_CC0pi_wc",
    &ev.sel_CC0pi_wc_, create, "sel_CC0pi_wc/O" );
  ...
  set_object_output_branch_address< TVector3 >( out_tree,
    "p3_mu_mcs", ev.p3_mu_mcs_, create );
  ...
  set_object_output_branch_address< TVector3 >( out_tree,
    "p3_lead_pi", ev.p3_lead_pi_, create );
  ...
  set_object_output_branch_address< TVector3 >( out_tree,
    "mc_p3_lead_pi", ev.mc_p3_lead_pi_, create );
...
set_object_output_branch_address< std::vector<float> >( out_tree,
 ...
 set_object_output_branch_address< std::vector<float> >( out_tree,
      "trk_bragg_mu_alt_dir_v", ev.trk_bragg_mu_alt_dir_v_, create );

set_object_output_branch_address< std::vector<bool> >( out_tree,
      "trk_bragg_p_fwd_preferred_v", ev.trk_bragg_p_fwd_preferred_v_, create );

 set_object_output_branch_address< std::vector<float> >( out_tree,
      "trk_bragg_p_alt_dir_v", ev.trk_bragg_p_alt_dir_v_, create );
...
EventCategory AnalysisEvent::categorize_event() {
  ...
  for ( size_t p = 0u; p < mc_nu_daughter_pdg_->size(); ++p ) {
    ...
    if ( pdg == MUON ) {
      ...
      if ( mom >= MUON_P_MIN_WC_MOM_CUT && mom <= MUON_P_MAX_MOM_CUT ) {
...
void AnalysisEvent::classify_tracks() {
  xgb_pid_vec_->clear();
  xgb_pid_vec_->resize(num_pf_particles_, (int) BDTClass::kInvalid);

  xgb_score_vec_->clear();
  xgb_score_vec_->resize(num_pf_particles_);
...
void AnalysisEvent::apply_selection() {
  ...
  for ( int p = 0; p < num_pf_particles_; ++p ) {
    ...
    if ( p == muon_candidate_idx_ ) {
      ...
      if ( sel_muon_contained_ ){
         if (track_length_->size() == 1 && trk_bragg_mu_fwd_preferred_v_->at(muon_candidate_idx_) == 0 && 6. < track_chi2_muon_->at(muon_candidate_idx_)){
      ...
      if ( muon_mom >= MUON_P_MIN_WC_MOM_CUT && muon_mom <= MUON_P_MAX_MOM_CUT ) {
    ...
    // Pion candidate
      ...
      float pi_mom;
      if ( end_contained ) {
        pi_mom = track_range_mom_mu_->at(p);
      }
      else {
      ...
      if (pi_mom > CHARGED_PI_WC_MOM_CUT) {
   ...
   /*if ( sel_muon_contained_ ) {
    ...
    if ( sel_muon_contained_ ) {
      muon_mom = track_range_mom_mu_->at( muon_candidate_idx_ );
          ...
          if (track_mcs_mom_mu_->at( muon_candidate_idx_ ) < 0.11){
              muon_mom = LOW_FLOAT;
          }
          else{
              muon_mom = track_mcs_mom_mu_->at( muon_candidate_idx_ ) - 0.0361*track_mcs_mom_mu_->at( muon_candidate_idx_ ) + 0.04;
          }
    }
    ...
    if ( !sel_muon_contained_ && p3mu.CosTheta() < -0.9){
        muon_mom = LOW_FLOAT;
        p3mu = TVector3( mu_dirx, mu_diry, mu_dirz );
        p3mu = p3mu.Unit() * muon_mom; 
    }
 ...
 const std::string& output_filename)
{
  ...
  assert(XGBoosterLoadModel(*booster, "./cc0pi_fstrack_pid_softprob.json") == 0);
